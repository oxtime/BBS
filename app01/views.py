...
def add_article(request):
    if request.method == "POST":
        ...
        if not request.user.blog:
            blog_obj = models.Blog.objects.create(site_name=request.user,site_title='此人很懒,没有签名',site_theme=str(request.user)+".css")
            article_obj = models.Article.objects.create(title=title, desc=desc, content=str(soup),
                                                     category_id=category_id, blog=blog_obj)
            models.UserInfo.objects.filter(username=request.user).update(blog=blog_obj)
        else:
...
def get_music(request,id):
    ...
    artice_obj = models.Article.objects.filter(pk=id).first()
    content = artice_obj.content
    res = ''
    for _char in content:
        if not '\u4e00' <= _char <= '\u9fa5':
            ...
            res += _char
    app_id = '17573545'
    api_key = 'y0cN82owiG82wVnrgqNHEgAf'
    Secret_Key = 'LshXKLnrq4PLkP6HVh7Oyai2m2Zhlaay'
    client = AipSpeech(app_id, api_key, Secret_Key)
    res = client.synthesis(res, 'zh', 1,
                           {
                               'vol': 5,
                               'spd': 2,
                               'pit': 5,
